    ...
    module InstanceMethods
      ...
      def record_create
        if paper_trail_switched_on?
          ...
          _data = merge_metadata(data)
          object = {action: _data[:event], object: self}
          PaperTrail.config.redis_it object
          send(self.class.versions_association_name).create! _data
        end
      end
      ...
      def record_update
        if paper_trail_switched_on? && changed_notably?
          ...
          _data = merge_metadata(data)
          PaperTrail.config.redis_it action: _data[:event], object: self
          send(self.class.versions_association_name).build _data
        end
      end
